<?php
/**
 * Plugin Thai lexicon topic
 *
 * @language th
 * @package modx
 * @subpackage lexicon
  
 * @author Mr.Kittipong Intaboot COE#18,KKU
 * @updated 2010-07-21
 */
$_lang['event'] = 'เหตุการณ์';
$_lang['events'] = 'เหตุการณ์';
$_lang['plugin'] = 'ปลั๊กอิน';
$_lang['plugin_add'] = 'เพิ่มปลั๊กอิน';
$_lang['plugin_add_to_event'] = 'เพิ่มปลั๊กอินให้เหตุการณ์';
$_lang['plugin_code'] = 'โค้ดปลั๊กอิน (php)';
$_lang['plugin_config'] = 'การตั้งค่าปลั๊กอิน';
$_lang['plugin_desc'] = 'คำอธิบาย';
$_lang['plugin_delete_confirm'] = 'คุณแน่ใจว่าต้องการลบปลั๊กอินนี้?';
$_lang['plugin_disabled'] = 'ปิดการใช้งานปลั๊กอิน';
$_lang['plugin_duplicate_confirm'] = 'คุณแน่ใจว่าต้องการทำสำเนาปลั๊กอินนี้?';
$_lang['plugin_err_create'] = 'เกิดข้อผิดพลาดขณะกำลังสร้างปลั๊กอิน';
$_lang['plugin_err_exists_name'] = 'ปลั๊กอินชื่อนี้มีอยู่แล้ว';
$_lang['plugin_err_invalid_name'] = 'ชื่อปลั๊กอินนี้ใช้ไม่ได้';
$_lang['plugin_err_nf'] = 'ไม่พบปลั๊กอิน!';
$_lang['plugin_err_ns'] = 'ไม่มีการระบุปลั๊กอิน';
$_lang['plugin_err_ns_name'] = 'กรุณาระบุชื่อปลั๊กอิน';
$_lang['plugin_err_remove'] = 'เกิดข้อผิดพลาดขณะกำลังลบปลั๊กอิน';
$_lang['plugin_err_save'] = 'เกิดข้อผิดพลาดขณะกำลังบันทึกปลั๊กอิน';
$_lang['plugin_event_err_duplicate'] = 'เกิดข้อผิดพลาดขณะที่พยายามทำสำเนาเหตุการณ์ของปลั๊กอิน';
$_lang['plugin_event_err_nf'] = 'ไม่พบเหตุการณ์ของปลั๊กอิน';
$_lang['plugin_event_err_ns'] = 'ไม่ระบุเหตุการณ์ของปลั๊กอิน';
$_lang['plugin_event_err_remove'] = 'เกิดข้อผิดพลาดขณะกำลังลบเหตุการณ์ของปลั๊กอิน';
$_lang['plugin_event_err_save'] = 'เกิดข้อผิดพลาดขณะกำลังบันทึกเหตุการณ์ของปลั๊กอิน';
$_lang['plugin_event_msg'] = 'เลือกเหตุการณ์ที่คุณต้องการให้ปลั๊กอินนี้เกิดการทำงาน';
$_lang['plugin_event_plugin_remove_confirm'] = 'คุณแน่ใจว่าต้องการลบปลั๊กอินนี้ออกจากเหตุการณ์นี้?';
$_lang['plugin_event_update'] = 'ปรับปรุงเหตุการร์ของปลั๊กอิน';
$_lang['plugin_lock'] = 'ป้องกันการแก้ไข';
$_lang['plugin_lock_msg'] = 'ผู้ใช้ต้องมีคุณสมบัติ edit_locked เพื่อที่จะแก้ไขปลั๊กอินนี้';
$_lang['plugin_locked_message'] = 'ปลั๊กอินนี้ถูกป้องกัน';
$_lang['plugin_management_msg'] = 'คุณสามารถเลือกปลั๊กอินที่คุณต้องการแก้ไข';
$_lang['plugin_msg'] = 'คุณสามารถเพิ่ม/แก้ไข ปลั๊กอิน แต่จำไว้ว่าปลั๊กอิน ต้องเป็นโค้ด PHP เท่านั้น ซึ่งจะทำงานทันทีเมื่อถึงเหตุการณ์ระบบที่เลือก';
$_lang['plugin_name'] = 'ชื่อปลั๊กอิน';
$_lang['plugin_new'] = 'สร้างปลั๊กอินใหม่';
$_lang['plugin_priority'] = 'แก้ไขปลั๊กอินโดยจัดการเรียงตามเหตุการณ์';
$_lang['plugin_properties'] = 'คุณสมบัติปลั๊กอิน';
$_lang['plugin_title'] = 'สร้าง/แก้ไข ปลั๊กอิน';
$_lang['plugin_untitled'] = 'ปลั๊กอินไม่มีชื่อ';
$_lang['plugins'] = 'ปลั๊กอิน';